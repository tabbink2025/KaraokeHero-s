'use client';
/* Shared primitives ported from the prototype's ui.jsx */
import React from 'react';
import type { Participant } from '@/lib/types';

export function Icon({ name, size = 22, sw = 1.8 }: { name: string; size?: number; sw?: number }) {
  const p = {
...
export const THEME_ICON: Record<string, string> = {
  party: 'ball', guilty: 'heart', rock: 'bolt', nl: 'diamond', duet: 'duo', movie: 'play',
};
...
export function avaClass(id: string): string {
  let h = 0;
  for (let i = 0; i < (id || '').length; i++) h = (h * 31 + id.charCodeAt(i)) % 6;
  return 'a' + (h + 1);
}

export function partName(participants: Participant[], id: string): string {
  return participants.find((p) => p.id === id)?.name ?? '???';
}

export function singerNames(participants: Participant[], ids: string[]): string {
  return ids.map((id) => partName(participants, id)).join(' & ');
}

export function Avatar({ id, name }: { id: string; name: string }) {
  return <span className={'ava ' + avaClass(id)}>{(name || '?').slice(0, 1).toUpperCase()}</span>;
}

export function AvaStack({ ids, participants }: { ids: string[]; participants: Participant[] }) {
  return (
    <span className="ava-stack">
      {ids.map((id) => <Avatar key={id} id={id} name={partName(participants, id)} />)}
    </span>
  );
}

export function Diff({ d }: { d: string }) {
